...
- **¿Un solo workflow de CI/CD, o se separan las responsabilidades?** → Tres workflows, cada uno con su disparador y su alcance de secretos:
  ...
  **A revisar al implementar (tareas 24-25)**: esta separación asumía push a GHCR con `GITHUB_TOKEN`, sin credenciales de Azure en `publish.yml` (`architecture.md` punto 12, revisado en `BAS-3` — ya no se usa GHCR, se acepta el ACR que aprovisiona el propio entorno de Container Apps). Empujar al ACR del entorno exige que `publish.yml` también se autentique contra Azure, lo que debilita la motivación original de mantenerlo sin credenciales de Azure. Alternativa a valorar: que `publish.yml` se limite a build + tests, y que el build/push de la imagen ocurra dentro del propio pipeline de `aspire deploy` (`build-web`/`push-web`, ya presentes en `aspire deploy --list-steps`), simplificando quizá a dos workflows en vez de tres.
