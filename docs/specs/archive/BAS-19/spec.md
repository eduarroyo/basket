...
- [x] Contiene 17 diapositivas, en español: portada (con marca BIGschool/Máster en Desarrollo con IA), contexto/problema (Trabajo de Fin de Máster), qué hace la app, capturas de portada+calendario, capturas de detalle de partido+clasificación, diagrama de arquitectura (con simplicidad deliberada), decisiones destacadas, infraestructura como código con Aspire, CI/CD (GitHub Actions), ciclo SDD, Claude Code multiagente con git worktrees, skills a medida, gobernanza, testing, estado actual, mejoras pendientes, cierre.
...
- **Iteración 3, con el PR #38 ya abierto**: el autor pidió quitar el framing "proyecto voluntario, sin financiación" (pasa a presentarse como Trabajo de Fin de Máster con vocación de convertirse en herramienta real para los juegos mancomunados), añadir una diapositiva de mejoras pendientes (BAS-4, UX, alta de administradores), mejorar la diapositiva de arquitectura (lista de tecnologías con viñetas + nota de simplicidad deliberada) y reforzar la de CI/CD — ver detalle en `plan.md`. De paso se diagnosticó y corrigió un bug real reportado por el autor: el diagrama Mermaid de arquitectura aparecía como un recuadro vacío en algunos casos, por una incompatibilidad conocida entre Mermaid (mide texto con `getBBox()`) y reveal.js (oculta diapositivas inactivas con `display:none`).
