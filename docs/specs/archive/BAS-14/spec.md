...
- **Suscripción "Desde URL" verificada en producción, con una limitación conocida de Google ajena a nosotros**: tras el despliegue a producción, se suscribió el feed real (`.../competiciones/1/calendario.ics`) en Google Calendar mediante "Desde URL" y el primer partido de prueba se mostró correctamente. Al añadir un segundo partido (otra jornada, otra fecha) para comprobar la actualización automática, Google Calendar no lo reflejó ni siquiera tras eliminar y volver a añadir la suscripción con la misma URL — mientras que el feed servido por el endpoint, comprobado por descarga directa (`curl`) en ese mismo momento, ya incluía el `VEVENT` nuevo. Es decir, la lentitud/no-actualización observada es enteramente del lado de Google (caché interna asociada a la URL, ajena al `Cache-Control`/TTL de nuestro Output Caching, con comportamiento errático documentado por otros usuarios en los foros de ayuda de Google Calendar), no un problema de nuestro feed ni de nuestra política de caché. No hay ninguna acción de nuestro lado que lo mitigue; como mucho, añadir un parámetro de query distinto a la URL al volver a suscribirse (p. ej. `?v=2`) puede hacer que Google la trate como una URL nueva y la consulte de inmediato, sin que el endpoint necesite cambiar nada (los parámetros de query no reconocidos se ignoran).
