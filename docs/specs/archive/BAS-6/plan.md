...
**Cookies y token antifalsificación**: cada test de alta/edición crea su propio `HttpClient` (`AppHostSqlFixture.CreateWebHttpClient()`) y reutiliza el mismo a lo largo del test (login → GET `Create` → POST `Create` → GET `Edit` → POST `Edit`) — el `SocketsHttpHandler` por defecto de `HttpClient` gestiona cookies por sí solo, sin `CookieContainer` manual. El token `__RequestVerificationToken` sí hay que extraerlo a mano del HTML de cada GET (una expresión regular simple) porque viaja como campo oculto del formulario, no como cookie — usar un token de una petición anterior falla con 400 (probado).

**"HttpClient nuevo" no es "sesión nueva"**: `CreateWebHttpClient()` no da un `CookieContainer` aislado por llamada, como se asumió al escribir el primer borrador de este plan — `IHttpClientFactory` reutiliza el mismo `HttpMessageHandler` (y con él, el mismo `CookieContainer`) entre llamadas al mismo nombre de recurso mientras ese *handler* siga vivo (por defecto, 2 minutos). Descubierto porque `PeticionAnonimaATemporadasRedirigeALogin` pasaba en local pero fallaba en CI: según el orden en que xUnit ejecutara los tests de la clase, esa petición "anónima" podía heredar la cookie de sesión de un test de login anterior sobre la misma fixture. Para los tests de alta/edición da igual (cada uno hace su propio login al principio, sobrescribiendo cualquier cookie previa), pero para comprobar acceso *anónimo* de verdad hace falta un cliente que no pase por ese *pool* — `AppHostSqlFixture.CreateAnonymousWebHttpClient()`, con `HttpClientHandler { UseCookies = false }` sobre el endpoint resuelto vía `DistributedApplication.GetEndpoint("web", "https")`.
