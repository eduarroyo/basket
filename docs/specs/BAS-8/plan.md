...
Test de integración (`tests/BasketBaseTracker.Tests/Integration/`), reutilizando `AppHostSqlFixture` y `AdminHttpTestHelpers` (BAS-6/BAS-7): crea el catálogo y el equipo necesarios dentro del propio test (`Temporada`, `Categoria`, `Club`, `Competicion`, `Equipo`), luego alta + edición + baja + listado de `FichaJugador`, más un test que da de alta dos fichas con el mismo dorsal en el mismo equipo y comprueba el mensaje de error. Misma cautela que en BAS-7 con la extracción de IDs (acotada a la fila, no al primer enlace de la página) y con no comparar texto acentuado contra HTML crudo en las aserciones.
