...
**Decisión**: ASP.NET Core Identity con autenticación por cookies. Los usuarios anónimos no requieren autenticación (consultas públicas); los administradores sí. La v1 tenía un único rol de administrador; `BAS-16` lo dividió en dos, al introducir import/export con su propia restricción de acceso (`functional.md`, `screens.md`): `GestorCompeticion` (todas las pantallas de gestión anteriores) y `Administrador` (administrador del sistema, solo import/export). El modelo sigue preparado para desglosarse más en el futuro (gestor de equipos, de resultados...), ver MF-7.
...
- **Alertas**: reglas de Azure Monitor sobre Application Insights, ligadas a los NFR de `functional.md`: latencia p95 > 200ms, tasa de error > 0,5% y fallo de health checks — **globales** por ahora (todas las peticiones), no desglosadas por endpoint (calendario/resultados/clasificación) como planteaba originalmente esta decisión, porque esas pantallas todavía no existen (`BAS-3` es solo infraestructura); revisar el alcance cuando existan. Canal de notificación: email, sin coste adicional y suficiente para un único administrador de sistema. No son un recurso de primera clase de Aspire (no hay un `AddAzureMonitorAlert` equivalente a `AddAzureApplicationInsights`) — se configuran directamente contra Azure con `az` una vez provisionados los recursos, no desde `AppHost.cs`. La latencia p95 y la tasa de error se implementan como *scheduled query rules* (KQL sobre Application Insights), no alertas de métrica simples: ni `requests/duration` ni `requests/failed` soportan agregación de percentil ni de ratio en la API de métricas de Azure Monitor. El fallo de health checks usa `RestartCount` de la Container App como *proxy* (no hay Availability Tests, ver más abajo).
...
- **Salvaguardas**, dado lo destructivo del reemplazo completo: confirmación explícita del administrador antes de ejecutar (no un único clic) y **backup automático del estado actual** justo antes de reemplazar, subido a un contenedor de **Azure Storage Blobs** (`BAS-16` — más robusto ante la pérdida del fichero descargado que devolver el backup solo al navegador, a cambio de un recurso de infraestructura nuevo; emulador Azurite en local/tests, mismo patrón que el resto de recursos Azure de `AppHost.cs`). Si falla la subida del backup, no se llega a importar.
...
- **Alcance de entidades**: todas las entidades catálogo y transaccionales de `data-model.md`. La clasificación queda fuera del fichero — al ser una consulta calculada y no una tabla (punto 14), no hay nada que exportar ni importar; se recalcula sola a partir de los `Partido` importados en cuanto se consulta. Las cuentas de Identity (`AspNetUsers`/`AspNetRoles`) tampoco entran — el fichero cubre los datos de la competición, no las cuentas de acceso al sistema.
- **Preserva el `Id` original de cada fila** al reimportar (`SET IDENTITY_INSERT` por tabla, en el orden de dependencia correcto — todas las FK son `DeleteBehavior.Restrict`, sin cascada) — un backup/restauración debe reproducir también los enlaces externos ya compartidos (URLs de partido, feeds iCal de `BAS-14`), no solo el contenido.
- **Acceso solo al rol `Administrador`** (sistema, punto 7) — el resto del área Admin exige `GestorCompeticion`.
...
**Decisión**: GitHub Actions — build, tests, publicación de la imagen en el Azure Container Registry del entorno de Container Apps (punto 12) y despliegue a Container Apps. Tres workflows con disparador y alcance de secretos distintos (detalle y motivación en `docs/specs/BAS-3/spec.md`): `ci.yml` (build + tests en cada push/PR a `develop`/`main`, sin secretos de Azure), `publish.yml` (`push` a `main`: build + tests + build de imagen + push al ACR vía SDK Container Support, sin Dockerfile) y `deploy.yml` (disparado por `publish.yml` vía `workflow_dispatch`, con el `image_tag` recién publicado). Autenticación contra Azure por OIDC (Federated Identity, sin secretos de cliente en claro) en ambos workflows que la necesitan.
...
- **Migraciones de base de datos**: se aplican como un paso explícito de `deploy.yml` (`dotnet ef database update`, login `sqladmin`) antes de desplegar la nueva revisión del Container App, en una única ejecución controlada — no al arrancar cada instancia de la aplicación, para evitar que varias réplicas intenten migrar a la vez en un pico de tráfico. El runner no tiene IP fija, así que el paso abre una regla de firewall temporal en Azure SQL para su propia IP pública y la cierra al terminar (`if: always()`).
- **Despliegue y promoción de tráfico**: Container Apps en modo de revisiones múltiples (`Multiple` revision mode, fijado en el propio Bicep vía `PublishAsAzureContainerApp` en `AppHost.cs` — no es un ajuste manual que un despliegue posterior pueda revertir). Cada ejecución de `deploy.yml` despliega la imagen nueva (`az containerapp update --image ... --revision-suffix ...`, no `azd deploy`) como revisión **sin tráfico**, corre el smoke E2E contra la URL específica de esa revisión, y solo si pasa promociona el 100% del tráfico a esa revisión por nombre (`az containerapp ingress traffic set`) — si el smoke E2E falla, la revisión nueva queda desplegada pero sin tráfico, sin necesidad de rollback.
- **Rollback**: revertir a una revisión anterior es un cambio manual de tráfico (misma orden que la promoción, apuntando a la revisión previa), sin reconstruir ni redesplegar nada — coste cero, porque una revisión inactiva en un plan de consumo no consume recursos. Riesgo sin mitigar: `dotnet ef database update` es *forward-only*, así que revertir a una imagen anterior a una migración de esquema puede dejar código antiguo corriendo contra un esquema incompatible — documentado, no bloqueante para este incremento.
...
Relacionado con el punto 7 (Autenticación y autorización). `functional.md` apunta a que el rol de administrador podría desglosarse más allá de la división `GestorCompeticion`/`Administrador` ya hecha en `BAS-16`, en roles todavía más específicos (gestor de equipos, de resultados...). ASP.NET Core Identity soporta roles de forma nativa (más `IdentityRole` y atributos `[Authorize(Roles=...)]`), así que el coste de introducirlos más adelante es bajo. Se descarta ir más allá de esa división ahora porque en la práctica sigue habiendo un único administrador de facto (con ambos roles) y diseñar una matriz de permisos más fina sin un caso de uso real que la motive sería especular sobre necesidades futuras — contrario a la filosofía de simplicidad del proyecto.
